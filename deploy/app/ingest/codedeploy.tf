...
aws deploy create-deployment \
    ...
    --application-name "${aws_codedeploy_app.ingest.name}" \
    --deployment-config-name CodeDeployDefault.ECSAllAtOnce \
    --deployment-group-name "${aws_codedeploy_deployment_group.ingest.deployment_group_name}" \
...
resource "aws_codedeploy_app" "ingest" {
...
resource "aws_codedeploy_deployment_group" "ingest" {
  app_name               = aws_codedeploy_app.ingest.name
  ...
  ecs_service {
    ...
    service_name = aws_ecs_service.ingest.name
  }
  deployment_style {
...
resource "local_file" "code_deploy_sh" {
  ...
  depends_on = [
    aws_codedeploy_app.ingest,
    aws_codedeploy_deployment_group.ingest,
    aws_ecs_task_definition.ingest
  ]
}
